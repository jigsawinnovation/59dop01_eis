...
<div id="menu-navbar" class="navbar-inner navbar-fixed-top">
  ...
  <span class="report-name" style="max-width: 70%"><?php echo $title;?></span>
    <ul class="nav navbar-top-links navbar-right">
      <li id="summary">
        <div onclick="getData('<?php echo site_url().report_url(); ?>gateway_report/gateway_summary');">
        <div class="font-icon-tab"><i class="fa fa-bar-chart" aria-hidden="true"></i></div>
        </div>
      </li>
      <li id="table">
        <div onclick="getData('<?php echo site_url().report_url(); ?>gateway_report/gateway_table');">
...
$(document).ready(function() {
    ...
    getData('<?php echo site_url().report_url(); ?>gateway_report/gateway_summary');
    getDataArea('<?php echo site_url().report_url(); ?>gateway_report/gateway_map');
});
</script>
